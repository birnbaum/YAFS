...
class AnimationTrack:
    def __init__(self, df_points, dpi=100, bg_map=True, aspect="equal", map_transparency=0.5, voronoi_points=[]):
        ...
        for i in range(len(df_points)):
            ...
            if bg_map:
                ...
                cells = [self.map[i].to_pixels(vertices[region]) for region in regions]

                cmap = plt.cm.Set3
                # We generate colors for districts using a color map.
                colors_districts = cmap(np.linspace(0.0, 1.0, len(voronoi_points)))[:, :3]

                # ax.imshow(ma.img, aspect='equal', alpha=0.8)
                self.axarr[i].add_collection(mpl.collections.PolyCollection(cells, facecolors=colors_districts, edgecolors="k", alpha=0.25))
    ...
    def computePoints(self, track_df=None, linewidth=0.5):
        warnings.warn(
            "The computePoints function is deprecated and " "will be removed in version 2.0.0. " "Use the compute_points function instead.",
            FutureWarning,
            stacklevel=8,
        )
    ...
    def computeTracks(self, linewidth=0.5):
        warnings.warn(
            "The computeTracks function is deprecated and " "will be removed in version 2.0.0. " "Use the compute_tracks function instead.",
            FutureWarning,
            stacklevel=8,
        )
    ...
    def makeVideo(self, linewidth=0.5, output_file="video", framerate=5):
        warnings.warn(
            "The makeVideo function is deprecated and " "will be removed in version 2.0.0. " "Use the make_video function instead.", FutureWarning, stacklevel=8
        )
    ...
    def makeMap(self, linewidth=2.5, output_file="map"):
        warnings.warn(
            "The makeMap function is deprecated and " "will be removed in version 2.0.0. " "Use the make_map function instead.", FutureWarning, stacklevel=8
        )
    ...
    def makeImage(self, linewidth=0.5, output_file="image", framerate=5, save_fig_at=None):
        warnings.warn(
            "The makeImage function is deprecated and " "will be removed in version 2.0.0. " "Use the make_image function instead.", FutureWarning, stacklevel=8
        )
    ...
    def isNewFrame(self, point, next_point):
        warnings.warn(
            "The isNewFrame function is deprecated and " "will be removed in version 2.0.0. " "Use the is_new_frame function instead.",
            FutureWarning,
            stacklevel=8,
        )
