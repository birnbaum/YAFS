...
if myConfiguration_ == "tinny":
    ...
    func_REQUESTPROB = (
        "random.random()/4"
    )  # Popularidad de la app. threshold que determina la probabilidad de que un dispositivo tenga asociado peticiones a una app. tle threshold es para cada ap
    func_REQUESTPROB = (
        "1.0"
    )  # Popularidad de la app. threshold que determina la probabilidad de que un dispositivo tenga asociado peticiones a una app. tle threshold es para cada ap
...
if myConfiguration_ == "isaac1":
    ...
    func_SERVICERESOURCES = (
        "random.randint(250,750)"
    )  # MB de ram que consume el servicio, teniendo en cuenta noderesources y appgeneration tenemos que nos caben aprox 1 app por nodo o unos 10 servicios
...
def placeAppInCommunityOLD(appId, clientId, candidateCommunity):
    ...
    orderedDevices = devicesFirstFitDescendingOrder(
    ...
    for appCommu in appsCommunities[appId]:
        ...
        if (
            appCommu[0] == appCommu[0] & servicesToPlace
        ):  # la comunidad de app que pillo ahora, tiene todos sus servicios pendientes de allocar? comunidad = comunidad INTERSCCION PENDIENTES
            if verbose_log:
...
if ILPoptimization:
    ...
    problem += pulp.lpSum([(UserServiceDevAssignment[i] * networkDelay(i)) for i in assignCombinations]), "Objective"
    ...
    for devId in fognodes:
        problem += (
            pulp.lpSum([(UserServiceDevAssignment[(usrservId, devId)] * myServicesResources[usrservId[1]]) for usrservId in userServices])
            <= myDevices[devId]["RAM"],
