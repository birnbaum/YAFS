...
class MCDARoutingAndDeploying(Selection):
    ...
    def runELECTRER(self, pathRScript, pathWD, fileName, criteriaWeights, IndifferenceThresholds, PreferenceThresholds, VetoThresholds, minmaxcriteria):
        ...
        cmd = [
            "Rscript",
            pathRScript + "rscripts/ELECTREv2.R",
            pathWD,
            fileName,
            criteriaWeights,
    ...
    def ELECTRE_evaluation(self, sim, node_src, nodes, message, app_name, service):
        ...
        df["hopcount"] = values
        criteriaMinMax += "min"
        q1 = np.percentile(values, 10) / 3.0
        ...
        df["latency"] = values
        criteriaMinMax += ",min"
        q2 = np.percentile(values, 10) / 3.0
        ...
        df["power"] = values
        criteriaMinMax += ",min"
        q3 = np.percentile(values, 10) / 3.0
        ...
        criteriaMinMax += ",min"
        q4 = np.percentile(values, 10) / 3.0
        ...
        df["cost"] = values
        criteriaMinMax += ",min"
        q5 = np.percentile(values, 10) / 3.0
        ...
        output = self.runELECTRER(
            ...
            pathWD=self.dname,
            fileName="/data_%i" % self.idEvaluation,
