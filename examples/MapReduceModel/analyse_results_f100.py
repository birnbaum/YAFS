...
def performResults(df, dfl, pathNetwork, f, exp, it, idcloud):
    ...
    try:
        ...
        for app, src in minmaxPATHs:
            ...
            valueMin = dmsg[dmsg["id"].isin(idMin)].groupby(["id"]).agg({"latency": np.sum}).mean().values[0]  # Include cloud?
            if math.isnan(valueMin):
                valueMin = 0
...
for i in range(nSimulations):
    for n in range(100, 301, 20):
        ...
        performResults(df, dfl, pathNetwork, fcsv, exps, i, n)
