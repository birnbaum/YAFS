...
def autolabel2(rects, hshift, a, b):
    ...
    for rect in rects:
        ...
        ax.text(
            ...
            1.05 * h + hshift,
            "%.0f" % float(float(a[i]) * 100.00 / float(b[i])) + "%",
            ha="center",
            va="bottom",
            size=12,
        )
...
def autolabel(rects, hshift):
    for rect in rects:
        h = rect.get_height()
        ax.text(rect.get_x() + rect.get_width() / 2.0, 1.05 * h + hshift, "%d" % int(h), ha="center", va="bottom", size=14)
    ...
    for metric_ in ["availabilitySensorWrites", "availabilityCloudReads"]:
        ...
        if metric_ == "availabilityCloudReads" and variableSizeOf == "-n":
            ax.legend(
                ...
                ("replica-aware", "single-file", "only-cloud-file", "fogstore", "total number of files"),
                fontsize=18,
                loc="lower right",
            )
        else:
