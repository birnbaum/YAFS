...
parser.add_argument("--work-dir", type=str, default="exp22/", help="Working directory")
...
# tuple: yLabel,
info_metrics = {
    "totalMSG": ("# transmitted packets (NO)"),
...
for metric in metrics:
    ...
    if len(ss1) != len(ss2):
        print("ERROR! - Series with different sizes")  # END?
    ...
    if metric == "totalMSG" or metric == "totalMSGsrcrep":  # or metric == "totalLATrepclo":
        ax.legend(
            (rects1[0], rects13[0], rects4[0], rects41[0], rects2[0]),
            ("replica-aware", "replica-aware (avg. per replica)", "fogstore", "fogstore (avg. per replica)", "single-file"),
            fontsize=16,
            ncol=2,
        )  # loc='lower right'
    #    elif :
...
ss1b, ss2b, ss3b, ss4b, ss5b, ss5b, ss6b = (
    defaultdict(list),
    defaultdict(list),
    defaultdict(list),
    defaultdict(list),
...
ax.legend(
    (rects1[0], rects2[0], rects5[0], rects6[0], rects3[0]),
    ("replica-aware min", "replica-aware max", "fogstore min", "fogstore max", "single-file"),
    fontsize=16,
    loc="lower right",
    ncol=6,
    framealpha=0.95,
