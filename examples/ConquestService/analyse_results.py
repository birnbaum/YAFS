...
def getRbyApp(df, dtmp):
    dr = pd.DataFrame(columns=["app", "user", "r", "time"])  # m - numero de mensajes enviados
...
resampleSize = 1000
tickGeneration = list(range(int(timeSimulation / resampleSize / 2.0), timeSimulation / resampleSize, int(timeSimulation / resampleSize / 2.0 / 10.0)))
...
for it in range(nsimulations):
    ...
    for g in list(dtmp.keys()):
        ...
        ticks = list(range(len(dr.time.values)))
        drs[(g[0], g[1])] = dr.time.values
        print("-" * 40)

    fig, ax = plt.subplots(figsize=(12.0, 4.0))
    for i, k in enumerate(drs):
        ax.plot(ticks, drs[k], "-", alpha=1.0, linewidth=2, label="S%i on '%i'" % (k[0], k[1]), color=color_sequence[i])
